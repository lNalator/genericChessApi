...
type BestMoveResult = { bestMove: string; ponder?: string | null };
...
export class StockfishService {
  ...
  async bestMove(fen: string, movetimeMs: number): Promise<BestMoveResult> {
    ...
    return new Promise((resolveResult, reject) => {
      ...
      const finish = (error?: Error, result?: BestMoveResult) => {
        if (settled) return;
        settled = true;
      ...
      const handleLine = (line: string) => {
        ...
        if (line === 'readyok' && !readySent) {
          ...
          analysisTimeoutId = setTimeout(() => finish(new GatewayTimeoutException('Stockfish timed out')), timeMs + TIMEOUT_BUFFER_MS);
          return;
        }
      ...
      // UCI handshake: uci -> uciok -> isready -> readyok -> position -> go -> bestmove.
      startupTimeoutId = setTimeout(() => finish(new GatewayTimeoutException('Stockfish handshake timed out')), STARTUP_TIMEOUT_MS);
      send('uci');
    });
  }
